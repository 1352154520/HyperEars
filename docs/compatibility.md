...
### 2.14 Samsung Galaxy Buds

| 设备范围 | 层级 | 证据 | 识别与确认 | 电量 | 噪声模式 |
|---|---|---|---|---|---|
| Galaxy Buds2 Pro（SM-R510） | 具体型号 | 公开实现 | 规范化型号名称或专属 RFCOMM UUID 选择候选；CRC 有效的状态帧确认能力 | 系统整机 → 私有组件 | 降噪、关闭、环境声 |
| Galaxy Buds2（SM-R177） | 具体型号 | 参考实现 | 规范化型号名称选择；合法扩展状态确认能力 | 系统整机 → 私有组件 | 降噪、关闭、环境声 |
| Galaxy Buds FE（SM-R400） | 具体型号 | 参考实现 | 规范化型号名称选择；合法扩展状态确认能力 | 系统整机 → 私有组件 | 降噪、关闭、环境声 |
| Galaxy Buds 共享服务回退 | 家族回退 | 家族外推 | 共享 RFCOMM UUID 选择；仅解析各代稳定字段 | 系统整机 → 私有组件 | 降噪、关闭、环境声 |

私有传输使用 Buds2 Pro RFCOMM UUID `2e73a4ad-332d-41fc-90e2-16bef06523f2`。合法扩展状态
确认后，卡片额外显示触控锁定和语音检测；环境声等级、均衡器、单耳降噪、无缝连接、耳缘
双击、侧音和超高环境声已进入型号控制接口。协议依据 GalaxyBudsClient 5.1.2 公开实现，
长按三级菜单提供左右耳动作选择，四级菜单提供两种噪声模式之间的切换组合；组合需要有效
设备回报才开放写入。Fix15 已由贡献者在 Buds2 Pro 与目标 HyperOS / MiLink 组合验证电量、
三态降噪、触控菜单与长按设置；Fix16 的连续写入合并和超时状态修正仍待同一设备复核。
Fix19 修复隐藏标题后的菜单状态通知中断，移除长按定时刷新；HyperOS 4.0 / Android 17
日志可确认旧版设备侧已生效，但 Fix19 的最终界面行为仍待实机复核。
后续 Fix19 实机仍报告同步提示不消失；Fix20 增加弹窗状态订阅及分阶段日志。
最终 Fix22 按用户要求移除长按同步提示及其显示计时器，保留内部请求目标和设备确认逻辑。
2026-09-20 用户报告 Fix22 实际测试可用（Buds2 Pro，HyperOS 4.0 / Android 17）；
这是最终版本整体反馈，不代表其他型号或全部边界场景均已验证，也不表示设备确认延迟已消除。
共享服务回退不开放触控、语音检测或其他型号设置；Buds2 和 Buds FE 的专属设置仍待实机
复核，若状态布局或固件修订不满足已知条件则保留基础电量与噪声能力。
